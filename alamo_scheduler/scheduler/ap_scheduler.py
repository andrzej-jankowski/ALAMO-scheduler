...
def tick(check, test):
    if 'check_{}'.format(test) == check['name']:
        print('{} Tick! Interval is: {}. The time is: {}'.format(check['name'], check['interval'] ,datetime.now()))

def update_check(scheduler, checks):
    # TODO Check if update flag is set in cache, Update check definitions if needed
...
def run_scheduler(checks):
    ...
    for k, v in checks.items():
        scheduler.add_job(tick, 'interval', seconds=v['interval'], max_instances=10,
                          id=str(k),
